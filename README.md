...
An offline, deterministic browser replay instrument with a live filter for Team Recalibrate's **SIH 2026 Problem Statement 26168**: *AI-ML based intelligent dead reckoning for seamless navigation*.

AstraNav-IDR demonstrates robust dead reckoning when GNSS becomes degraded or denied (e.g., in urban canyons, tunnels, or under jamming). The system combines a live **ES-EKF (Error State Extended Kalman Filter) with 15 states**, a **learned motion mode virtual odometer**, **Nonholonomic Constraints (NHC)**, **ML gated ZUPT and ZARU**, **NIS GNSS integrity gating**, a **systematic heading protection bound inspired by SBAS**, and a **Viterbi HMM route topology matcher**.
...
| :--- | :--- | :--- |
| **Motion Mode Classifier** | Python + NumPy/Pandas: multinomial logistic regression / softmax classifier (`iovnbd-model.json`), 4 classes, rotation invariant multiscale window features | ONNX Runtime / LiteRT inference on device |
| **Navigation Core** | Live ES-EKF in the browser with 15 states and quaternion attitude representation (`src/engine/esekf.ts`, `src/engine/liveeskf.ts`) | C++ 100 to 200 Hz edge engine with native Android bindings |
...
| **Integrity Envelope** | $2\sigma$ style covariance envelope plus a systematic heading protection bound inspired by SBAS ($\bar{v} \cdot \psi_{\text{sys}} \cdot \tau$) | Formal SBAS / ARAIM multihypothesis protection level |
...
> **Mechanization Note:** The live replay runner executes an ES-EKF core with 15 states and recorded gyro, compass, GNSS, and pseudo measurement aiding; horizontal phone accelerometer propagation is intentionally suppressed in this replay adapter to reflect real smartphone IMU noise limits identified during research.
...
1. **Live ES-EKF with 15 states (`src/engine/esekf.ts`, `src/engine/liveeskf.ts`):**
   ...
   - Live execution on every replay tick in the browser.
...
3. **ML Gated ZUPT and ZARU:**
   ...
   - Duration gating over 2 seconds prevents false stops during smooth highway cruising.
...
5. **Dual Integrity Envelope:**
   - Combines a $2\sigma$ style covariance envelope with a systematic heading protection bound inspired by SBAS ($\bar{v} \cdot \psi_{\text{sys}} \cdot \tau$).
...
1. **Sensor Capture (`/byod.html`):** A standalone web capture page running on any phone records device motion (accelerometer and gyroscope on three axes) and high accuracy GPS fixes locally.
2. **On Device Replay:** Sensor captures load into AstraNav, where the ES-EKF in the browser (15 states) calibrates its own gyro biases and navigates the user's own physical sensor stream.
...
- **Navigate:** Realtime map replay (2σ covariance ellipse, INS/Classical/EKF/Route match trails, GNSS fix status).
